...
public class AxisAlignedBoundingBox extends Intersectable implements Boundable {
    ...
    public List<Double> dry(double dirCoordinate, double pointCoordinate, double xMax, double xMin) {
        if (isZero(dirCoordinate)) {
            // if the point's T value is in the box,
            if (maxX >= pointCoordinate && minX <= pointCoordinate) {
                xMax = Double.MAX_VALUE;
                xMin = Double.MIN_VALUE;
            } else
                return List.of(0.0, xMin, xMax);
        }
        // if the vector's T coordinate is not zero, we need to check if we have values
        // where (MaxX - TPoint) / TDir > (MinT - TPoint) / TDir
        else {
            double t1 = (maxX - pointCoordinate) / dirCoordinate;
            double t2 = (minX - pointCoordinate) / dirCoordinate;
            xMin = Math.min(t1, t2);
            xMax = Math.max(t1, t2);
        }
        return List.of(1.0, xMin, xMax);
    }
